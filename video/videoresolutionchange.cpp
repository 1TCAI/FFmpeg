...
VideoResolutionChange::VideoResolutionChange(char * inFile,char * outFile, char *resolut)
    :inFilePath(inFile),outFilePath(outFile),resolution(resolut)
{
    cout<<"VideoResolutionChange begin"<<endl;
    init();
}
int VideoResolutionChange::init()
{
...
VideoResolutionChange::~VideoResolutionChange()         //感觉一些释放不能放析构函数内，除非构造函数调用init。
...
//    buffer = (uint8_t *)av_malloc(avpicture_get_size(CodeCtx->pix_fmt,width,height));
    buffer = (uint8_t *)av_malloc(av_image_get_buffer_size(CodeCtx->pix_fmt,width,height,1));

    ret = av_image_fill_arrays(dstFrame->data,dstFrame->linesize,buffer,CodeCtx->pix_fmt,width,height,1);
    if(ret < 0)
    {
...
int VideoResolutionChange::decode(AVCodecContext *codeCtx,AVPacket *pkt,AVFrame *dstFrame,struct SwsContext *c)
{
    int ret = -1;
    AVFrame *frame;
